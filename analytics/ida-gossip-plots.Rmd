...
```{r, echo=FALSE, fig.width=4, fig.height=4, eval=parallel_send_plot}

##### DELETE THIS FILTERS
#df_chunks <- df_chunks %>% filter( ParallelSendCount >= 4 )
#df_message_delivery <- df_message_delivery %>% filter( ParallelSendCount >= 4 )
#df_network_usage <- df_network_usage %>% filter( ParallelSendCount >= 4 )
######## End of Delete

##### ParallelSendCount  Plots ######

target_variable <- as.name("ParallelSendCount")
x_label="Dissemination Concurrency"
...
p <- p + theme(text = element_text(family="Times", face="plain", size=11), strip.text.y=element_text(family="Times", face="plain", size=12))
...
```{r, echo=FALSE, fig.width=5, fig.height=6, eval=!parallel_send_plot}
